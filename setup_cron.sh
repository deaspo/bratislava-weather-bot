...
# Create cron job entries for each city at different minutes to spread the load
...
# Alternative: Single multi-city update (all cities at once)
MULTI_CITY_CRON="0 * * * * cd $SCRIPT_DIR && $PYTHON_PATH $MAIN_SCRIPT --mode multi-city >> logs/multi_city_cron.log 2>&1"

echo "Choose deployment mode:"
echo "1) Separate updates - Each city posts at different times (recommended)"
echo "2) Batch updates - All cities post together"
echo "3) Both separate and batch updates"
