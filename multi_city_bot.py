...
class MultiCityWeatherBot:
        ...
        try:
            ...
            for city in self.cities:
                ...
                message = self.formatter.format_current_weather(
                    current_weather, 
                    city_name,
                    custom_hashtags=city.get("hashtags", [])
                )
    ...
    def post_current_weather_city(self, city_name):
        ...
        try:
            ...
            weather_service = self.weather_services[city_name]
            city_config = next(city for city in self.cities if city["name"] == city_name)
    ...
    def test_all_cities(self):
        ...
        for city in self.cities:
            ...
            if current_weather:
                logger.info(f"✅ {city_name}: {current_weather['temperature']}°C, {current_weather['description']}")
            else:
